#include <Siv3D.hpp>
enum Status { _start, _select, _tutorial, _course, _game_over, _submit, _ranking };
...
void Main() {
	Status status = _start, button_status = _tutorial;
	...
	while (System::Update()) {
		...
		switch (status) {
			...
			case _select:
				...
				font.draw_center(30, U" はい ", Scene::Center() * Vec2(0.5, 1.5), ColorF(0, 0, 0), 1.5, button_status == _tutorial ? Color(160, 216, 239, time.isRunning() ? Periodic::Square0_1(0.2) * 255 : 255) : Color(255, 255, 255));
				font.draw_center(30, U"いいえ", Scene::Center() * Vec2(1.5, 1.5), ColorF(0, 0, 0), 1.5, button_status == _course ? Color(160, 216, 239, time.isRunning() ? Periodic::Square0_1(0.2) * 255 : 255) : Color(255, 255, 255));
				if (KeyRight.down()) button_status = _course;
				if (KeyLeft.down()) button_status = _tutorial;
				if (KeyZ.down()) {
					clock = 0.2s;
					time.start();
				}
